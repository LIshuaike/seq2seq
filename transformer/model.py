...
class Encoder(nn.Module):
    ...
    def forward(self, src_seq, src_mask, return_attns=False):
        ...
        enc_output = self.dropout(self.position_enc(
        ...
        enc_outpu = self.layer_norm(enc_output)
...
class Decoder(nn.Module):
    ...
    def forward(self,
                ...
                src_mask,
                return_attns=False):
        dec_self_attn_list, enc_dec_attn_list = [], []

        dec_output = self.dropout(self.position_enc(
            self.trg_word_emb(trg_seq)))

        for dec_layer in self.layer_stack:
            dec_output, dec_self_attn, enc_dec_attn = dec_layer(
                dec_output,
                enc_output,
                self_attn_mask=trg_mask,
                enc_dec_attn_mask=src_mask)
        ...
        if return_attns:
            return dec_output, dec_self_attn_list, enc_dec_attn_list
        return dec_output
...
class Transformer(nn.Module):
    ...
    def __init__(self,
        ...
        if trg_emb_prj_weight_sharing:
            # share the weight between target word embedding and last dense layer
